...
describe('lambda handler', () => {
    ...
    test('omits header when cookie is missing', async () => {
        await handler(event, null as any, null as any)
        expect(event.Records[0].cf.request.headers).toEqual({})
    })

    test('omits header when cookie is malformed', async () => {
